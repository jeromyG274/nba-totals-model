#!/usr/bin/env python3
"""
Large-scale backtest: 1000 realistic NBA games with all 4 enhancements.
Generates synthetic but realistic game data based on known NBA parameters.
"""
...
# Team efficiency ratings (offensive efficiency)
TEAM_OFFEFF = {
...
# Team defensive efficiency (points allowed)
TEAM_DEFEFF = {
...
def generate_1000_games():
    """Generate 1000 realistic NBA games."""
    games = []
    np.random.seed(42)
...
def run_large_backtest(num_games=1000, lookback_window=20):
    """Backtest model on 1000 games."""
    games = generate_1000_games()
    print(f"Generated {len(games)} realistic games")
    print(f"Total range: {min([g['total_pts'] for g in games])}-{max([g['total_pts'] for g in games])} pts\n")
    
    # Pre-fetch external maps once to avoid repeated network calls
    pace_map = get_team_pace_from_espn()
    injuries_map = get_team_injuries()

    results = []
    
    for i, current_game in enumerate(games):
        if i < lookback_window:
            training_games = games[:i]
        else:
            training_games = games[i-lookback_window:i]
        
        if len(training_games) < 4:
            continue
        ...
        try:
            ...
            pace_adj = get_pace_adjusted_total(predicted, current_game['away'], current_game['home'], pace_map)
            predicted += pace_adj
            ...
            if edge != 0:
                if (predicted_over and actual_over) or (not predicted_over and not actual_over):
                    result = "WIN"
                elif (predicted_over and not actual_over) or (not predicted_over and actual_over):
                    result = "LOSS"
                else:
                    result = "PUSH"
            else:
...
def summarize_large_backtest(results_df):
    ...
    print("\n" + "=" * 80)
    print("LARGE-SCALE BACKTEST (1000 GAMES)")
    print("=" * 80)
...
if __name__ == "__main__":
    print("Starting 1000-game backtest with all 4 enhancements...\n")
    results = run_large_backtest(num_games=1000, lookback_window=50)
    summary = summarize_large_backtest(results)
    results.to_csv("backtest_1000_results.csv", index=False)
    print("\n✓ Results saved to backtest_1000_results.csv")
